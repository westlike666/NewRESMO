...
if vectorize    
%% start vectorizing  
    %this approach is in parellel with for loop approach. The physical
    %meaning of each term is indicated in the for loop below
...
%%
    ...
    dT=(Ry*sum(sum(D_NDEN_OLD./NL.^2.*VOL))-1.5*kB*T*sum(sum(D_EDEN_OLD.*V)) ...
        -sum(sum(0.98263e-20*V.*eden.*(UX.*D_UX + UY.*D_UY + UZ.*D_UZ)))/3  )... % where is 0.98263 from. should be  4.9826e-26/1e12*1e18
