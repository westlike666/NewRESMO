...
%from 0 to r and d_theta from 0 to 2pi to find total volume at r(theta),
%the diff gave the volume of slices betwenn shell boundaries

V=2*pi/3*sin(phi)*d_phi*(diff(r.^3)); % N*1 colume volume of each shell

dV=2*pi*sin(phi)*d_phi*(diff(r.^2.*U)); 
